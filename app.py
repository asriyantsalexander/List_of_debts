...
def extract_card(pdf_path):
    ...
    for blk in blocks:
        mb = re.match(
            r'(Январь|Февраль|Март|Апрель|Май|Июнь|Июль|Август|'
            r'Сентябрь|Октябрь|Ноябрь|Декабрь)\s+(\d{4})', blk)
        if not mb: continue
        sd = re.findall(r'Сальдо на (\d{2}\.\d{2}\.\d{4})', blk)
        if not sd: continue
        pm = re.search(r'Сальдо пени на конец периода' + vp*(n_cols+1), blk)
        if pm:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            if fname_lower.endswith('.rar'):
                subprocess.run(
                    ['unrar', 'x', archive_path, extract_dir],
